...
let unsafeFromParts = (a, b, c, d, e) => UUID(List.String.joinWith("-", list{a, b, c, d, e}))

let show: t => string = (UUID(str)) => str

let hexDigits: int => Parser.t<string> = count => {
  open Parser
  \"<#>"(times(count, anyHexDigit), List.String.join)
}
    ...
    \"<*"(
      \"<*>"(
        \"<*"(
          \"<*>"(
            \"<*"(
              \"<*>"(\"<*"(\"<$>"(unsafeFromParts, hexDigits(8)), hyphen), hexDigits(4)),
              hyphen,
            ),
            hexDigits(4),
          ),
