...
describe("Tutorial", () => {
  ...
  test("Running a parser normally (success)", () => {
    ...
    anyDigit->runParser("3", _)->expect->toEqual(Result.Ok("3"))
  })
  ...
  test("Running a parser normally (failure)", () => {
    ...
    ->runParser("!", _)
    ->expect
    ->toEqual(Result.Error(ParseError.ParseError("Expected a digit, but found character '!'")))
  })
  ...
  test("tuple2 example", () => {
    open ReludeParse.Parser
    tuple2(anyDigit, anyDigit)->runParser("12", _)->expect->toEqual(Result.Ok(("1", "2")))
  })
  ...
  test("*> example", () => {
    open ReludeParse.Parser
    \"*>"(ws, anyDigit)->runParser("    3", _)->expect->toEqual(Result.Ok("3"))
  })
  ...
  test("pure example", () => {
    open ReludeParse.Parser
    pure(3)->runParser("abcdef", _)->expect->toEqual(Result.Ok(3))
  })
  ...
  test("Sequence with fail example", () => {
    ...
    ->runParser("9abc", _)
    ->expect
    ->toEqual(Result.Error(ParseError.ParseError("The count cannot be >= 5")))
  })
  ...
  test("<|> error example", () => {
    ...
    ->expect
    ->toEqual(
      Result.Error(ParseError.ParseError("Expected any alphabet letter (upper or lowercase)")),
    )
  })
  ...
  test("Custom error example", () => {
    ...
    ->runParser("abc", _)
    ->expect
    ->toEqual(Result.Error(ParseError.ParseError("Expected one or more digits")))
  })
  ...
  test("CSV example", () => {
    ...
    let expected = Result.Ok(list{"abc", "def"})
