...
describe("NanpPhone", () => {
  ...
  test("1 (333) 444-5555 (country code: 1, succeeds)", () =>
    expect(parse("1 (333) 444-5555")) |> toEqual(Result.ok(tuple3334445555))
  )
  ...
  test("empty string (fails)", () => expect(NanpPhone.parse("") |> Result.isError) |> toEqual(true))
  ...
  test("parseOption (success)", () =>
    expect(NanpPhone.parseOption("333.444.5555")) |> toEqual(
      Some(NanpPhone.unsafeFromString("333.444.5555")),
    )
  )

  test("parseOption (fails)", () => expect(NanpPhone.parseOption("1234567890")) |> toEqual(None))

  test("unsafeFromString (throws on failure)", () =>
    expect(() => NanpPhone.unsafeFromString("")) |> toThrow
  )
